...
// Introduction
//
// This symbol table library supplies basiata.  Plenty of room for
...
//    This means the void * cannot have zec insert and lookup for
// symbols linked to void * pointers of dro as a legal value.
//
// A main() is commented out and has testing code in it.
//
// Robert Heckendorn   Oct 14, 2020
//
...
void SymbolTable::enter(std::string name)                    
{
    ...
    stack.back()->debug(this->debugFlg);
}
...
//Tests if the current scope is the global scope. Returns true if it is and false if not
bool SymbolTable:: topIsGlobal() {
    return stack[stack.size()-1]->isGlobal();
}
