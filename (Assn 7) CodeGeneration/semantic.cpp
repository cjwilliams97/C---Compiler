...
#include <string>
#include <stack>
extern struct TreeNode* prototypeTreeRoot;
int numWarnings;
...
std::stack<std::string> scopeStack; //stack of scope names for local static variable allocation (name mangling)
void semanticAnalysis(struct TreeNode *syntaxTree, SymbolTable *symbolTable)
{
    ...
    */
    ...
    functionNode = newDeclNode(FuncDeclK, Integer, functionId, NULL, NULL, NULL, false);
                        ...
                        if (t->child[0] != NULL) {
                            if(t->child[0]->isArray == true) {
                                if (t->child[0]->expType == Char) {
                                    ...
                                    t->child[0]->size = strlen(t->child[0]->attr.string) + 1; // + 1 to account for size, stored 1 address before string start address
                        ...
                        if (t->isArray == true) { //variable declaration is an array
                            if (t->isScoped == true) {
                                if (t->isStatic == true) {
                                    ...
                                    s.append("-");
                                    s.append(scopeStack.top());
                                    symbolTable->insertGlobal(s, (TreeNode*) t);
                        ...
                        if (t->child[1]->kind.ExpK == ConstantK) {
                            if(t->child[1]->isArray == true) {
                                if (t->child[1]->expType == Char) {
                                    ...
                                    t->child[1]->size = strlen(t->child[1]->attr.string) + 1; // + 1 to account for size, stored 1 address before string start address
                    ...
                    if (t->expType == Char) {
                        if (t->isArray == true) {
                            if (t->strMemoryAllocated == false) {
                                ...
                                t->nodeMemType = Global;
                                t->size = strlen(t->attr.string) + 1; // + 1 to account for size, stored 1 address before string start address
                                t->offset = goffset - 1;
                                goffset -= t->size;  
                            }
                        }
