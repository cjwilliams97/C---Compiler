...
decl                : varDecl {$$ = $1;}
                    ...
                    ;
/*----------------------------------------------------------*/
varDecl             : typeSpec varDeclList ';' {$$ = setNodeType($1, $2, false); yyerrok;}
...
varDeclInit         : varDeclId {$$ = $1;}
                    | varDeclId ':' simpleExp {if ($1 != NULL) {$1->child[0] = $3; $1->isInitialized = true;} $$ = $1;}
                    | error ':' simpleExp {$$ = NULL; yyerrok;}
                    | varDeclId ':' error {$$ = NULL; yyerrok;}
                    ;

varDeclId           : ID {$$ = newDeclNode(VarDeclK, Void, $1, NULL, NULL, NULL, false);}
                    | ID '[' NUMCONST ']' {$$ = newDeclNode(VarDeclK, Void, $1, NULL, NULL, NULL, true); $$->size = $3->numValue + 1;}
                    | ID '[' error {$$ = NULL;}
                    | error ']' {$$ = NULL; yyerrok;}
                    ;
...
matched             : IF simpleExp THEN matched ELSE matched {$$ = newStmtNode(IfK, $1, NULL, $2, $4, $6); $$->hasMatchingElse = true;}
                    | otherStmt {$$ = $1;}
...
unmatched           : IF simpleExp THEN matched {$$ = newStmtNode(IfK, $1, NULL, $2, $4, NULL);}
                    ...
                    | IF simpleExp THEN matched ELSE unmatched {$$ = newStmtNode(IfK, $1, NULL, $2, $4, $6); $$->hasMatchingElse = true;}
                    ...
                    | IF error THEN matched ELSE unmatched    { $$ = NULL; yyerrok; $$->hasMatchingElse = true;}
                    ;
...
breakStmt           : BREAK ';' {$$ = newStmtNode(BreakK, $1, NULL, NULL, NULL, NULL);}
                    ;
/*----------------------------------------------------------*/
exp                 : mutable '=' exp {$$ = newExpNode(AssignK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mutable ADDASS exp {$$ = newExpNode(AssignK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mutable SUBASS exp {$$ = newExpNode(AssignK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mutable MULASS exp {$$ = newExpNode(AssignK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mutable DIVASS exp {$$ = newExpNode(AssignK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mutable INC {$$ = newExpNode(AssignK, $2, $1, NULL, NULL); $1->isLhsInExp = true;}
                    | mutable DEC {$$ = newExpNode(AssignK, $2, $1, NULL, NULL); $1->isLhsInExp = true;}
                    | simpleExp {$$ = $1;}
...
simpleExp           : simpleExp OR andExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | simpleExp OR error {$$ = NULL;}
                    | andExp {$$ = $1;}
                    ;

andExp              : andExp AND unaryRelExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | unaryRelExp {$$ = $1;}
...
relExp              : minmaxExp relop minmaxExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | minmaxExp {$$ = $1;}
...
minmaxExp           : minmaxExp minmaxop sumExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | sumExp {$$ = $1;}
                    ;
...
sumExp              : sumExp sumop mulExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | mulExp {$$ = $1;}
...
mulExp              : mulExp mulop unaryExp {$$ = newExpNode(OpK, $2, $1, $3, NULL); $1->isLhsInExp = true;}
                    | unaryExp {$$ = $1;}
...
mutable             : ID {$$ = newExpNode(IdK, $1, NULL, NULL, NULL);}
                    | ID '[' exp ']' {$$ = newExpNode(IdArrK, $2, newExpNode(IdK, $1, NULL, NULL, NULL), $3, NULL); $$->child[0]->isIndexed = true;}
                    ;
...
argList             : argList ',' exp {$$ = setSibling($1, $3); $3->isCallArg = true; yyerrok;}
                    | exp {$$ = $1; $$->isCallArg = true;}
                    | argList ',' error {$$ = NULL;}
                    ;
...
int main(int argc, char **argv)
{
    ...
    extern char *optarg;
    extern int optind;
    int dflg, pflg, Pflg, hflg, Sflg, Mflg;
    int errflg;
    char *ofile;
    char *infile;
    errflg = dflg = pflg = Pflg = hflg = Sflg = Mflg = 0;
    ofile = NULL;
    infile = NULL;
    char* outFileName;
    while (1) {
        ...
        while ((c = ourGetopt(argc, argv, (char *)"dpPMo:")) != EOF)
            switch (c) {
                case        'd':
                    dflg = 1;
                    break;
    ...
    if (infile) {
        if ((yyin = fopen(infile, "r"))) {
            ...
            outFileName = strdup(infile);
            outFileName[strlen(outFileName) - 2] = 't';
            outFileName[strlen(outFileName) - 1] = 'm';
        }
        else {
            // failed to open file
            numErrors++;
    ...
    if (Pflg == 1 && numErrors == 0) {
            PrintTree(syntaxTree, 0, -1, 0, false);
    }
    if (Mflg == 1 && numErrors == 0) {
        PrintTree(syntaxTree, 0, -1, 0, true);
        printf("Offset for end of global space: %d\n", goffset);
    }
    printf("Number of warnings: %d\n", numWarnings);
    printf("Number of errors: %d\n", numErrors);
    if (numErrors == 0) {
        generateCode(infile, outFileName, syntaxTree, symbolTable);
    }
